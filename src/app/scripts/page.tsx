...
export default async function ScriptsPage() {
  ...
  return (
    <>
      ...
      {workflows.length || hasApprovedDraft ? <Panel title="Experimental production workflows" description="Private rendering and upload status. Provider availability may vary; publishing remains a separate human action.">
        ...
        <div className="stack-md">{workflows.map((workflow) => <div className="workflow-row" key={workflow.id}>
          ...
          {workflow.status === "failed" && workflow.errorCode === "PROVIDER_CREDITS_DEPLETED" ? <><span className="error-text">Paid provider credits depleted. The workflow can be retried with the free image provider.</span>{workflow.attempts < 10 ? <RetryWorkflowButton id={workflow.id} /> : null}</> : workflow.status === "failed" && workflow.attempts < 10 ? <RetryWorkflowButton id={workflow.id} /> : workflow.status === "failed" ? <span className="error-text">Retry limit reached</span> : null}
